...
export default function NewUsersCard() {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm flex flex-col h-[52vh]">
      ...
      {/* Join Today Card */}
      <div className="mt-44">  {/* Changed from mt-50 (invalid value) to mt-12 */}
  <div className="pb-3">
    <h1 className="text-sm font-medium text-gray-900">Join Today</h1>
  </div>
  <div className="space-y-4">
    <div className="flex items-center -space-x-4">  {/* Increased negative space */}
      {visibleUsers.map((user) => (
        <div key={user.id} className="relative">
          <Avatar className="h-16 w-16 border-4 border-white hover:z-10 transition-all hover:scale-110">
            <AvatarImage
              src={`${user.avatar.split('?')[0]}?w=128&h=128&fit=crop&crop=faces&auto=format&q=80`}
              alt={user.name}
      ...
      {hiddenUsersCount > 0 && (
        <div className="relative">
          <span className="bg-blue-500 text-white rounded-full h-16 w-16 flex items-center justify-center text-sm font-medium border-4 border-white">
            +{hiddenUsersCount}
          </span>
        </div>
      )}
