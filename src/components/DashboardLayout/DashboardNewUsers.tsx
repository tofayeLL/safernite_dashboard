...
export default function DashboardNewUsers() {
  ...
  console.log("all user data", allStatsData);

  const profileImages = allStatsData?.result?.profileImages || [];
  const userCount = allStatsData?.result?.totalUsers || 0;
  const hiddenUsersCount = expanded 
    ? 0 
    : Math.max(profileImages.length - VISIBLE_USERS_LIMIT, 0);
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-2xl flex flex-col shadow-xs h-full">
      ...
      <div className="flex-1 flex flex-col justify-end">
        <div className="py-2">
          <h2 className="text-sm font-medium text-gray-900">Join {activeTab}</h2>
        </div>
        ...
        {profileImages.length > 0 ? (
          <div className="space-y-4">
            ...
            <div className="flex items-center -space-x-2 sm:-space-x-3 flex-wrap">
              {(expanded ? profileImages : profileImages.slice(0, VISIBLE_USERS_LIMIT)).map((imageUrl: string, index: number) => (
                <div key={`${imageUrl}-${index}`} className="relative group">
                  <Avatar className="h-12 w-12 sm:h-16 sm:w-16 border-2 sm:border-4 border-white hover:z-10 transition-all hover:scale-110 cursor-pointer">
                    <AvatarImage
                      src={`${imageUrl.split("?")[0]}?w=128&h=128&fit=crop&crop=faces&auto=format&q=80`}
                      alt={`User ${index + 1}`}
            ...
            {profileImages.length > VISIBLE_USERS_LIMIT && (
              <button
